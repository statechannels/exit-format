 ...
 * @param allocations the outcome containing at least one guarantee(s) which will be claimed for each asset.
...
export function claim(
  ...
  const afterClaimGuarantee: Exit = _.cloneDeep(allocations);
  ...
  for (let assetIndex = 0; assetIndex < allocations.length; assetIndex++) {
    const allocationsForOneAsset = allocations[assetIndex].allocations;
    ...
    if (
      allocationsForOneAsset[targetChannelIndex].allocationType !==
    ...
    const allocationsBeforeOurs = allocationsForOneAsset.slice(
      0,
      targetChannelIndex
    );
    const amountBeforeUs = sumAllocationAmounts(allocationsBeforeOurs);
    ...
    const maxAmountToPayOut = min(
      maxAmountCanPayOut,
      BigNumber.from(allocationsForOneAsset[targetChannelIndex].amount)
    );
    ...
    const {
      ...
      allocationsForOneAsset[targetChannelIndex],
      targetAllocations,
      maxAmountToPayOut,
